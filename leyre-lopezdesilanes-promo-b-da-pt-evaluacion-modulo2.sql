...
FROM customer AS c
LEFT JOIN rental AS r -- por si hay algún cliente que no haya alquilado películas
USING (customer_id)
...
WITH MoviesRented AS (
	SELECT cat.name AS CategoryName, rental_id
	FROM category AS cat
	INNER JOIN film_category USING (category_id)
	INNER JOIN inventory USING (film_id)
	INNER JOIN rental AS r USING (inventory_id))

SELECT CategoryName, COUNT(rental_id) AS MoviesRented
FROM MoviesRented
...
SELECT rating, ROUND(AVG(length), 0) AS AverageLength
FROM film
GROUP BY rating
...
SELECT CONCAT(a.first_name, ' ', a.last_name) AS ActorName, COUNT(fa.film_id) AS Movies
FROM actor AS a
INNER JOIN film_actor AS fa USING (actor_id)
GROUP BY actor_id -- se agrupa por el id del actor
